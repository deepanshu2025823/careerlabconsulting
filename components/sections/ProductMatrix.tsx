...
const services = [
  { 
    ...
    icon: <Bot aria-hidden="true" className="w-6 h-6" />, 
  ...
  { 
    ...
    icon: <LineChart aria-hidden="true" className="w-6 h-6" />, 
...
const ServiceCard = memo(({ service }: { service: typeof services[0] }) => (
  <article className="group relative block p-[1px] rounded-[2.5rem] bg-gradient-to-b from-white/10 to-transparent hover:from-blue-500/50 transition-all duration-500 cursor-pointer">
    <div className="relative h-full bg-[#0b0f1a] rounded-[2.5rem] p-8 overflow-hidden">
      <div className="flex justify-between items-center mb-8">
        <div className={`p-3 rounded-2xl ${service.bg} text-white`} aria-hidden="true">
          {service.icon}
        </div>
        <div className="text-[10px] font-bold px-3 py-1.5 rounded-full bg-blue-900/40 text-blue-300 border border-blue-500/30 uppercase tracking-widest">
          {service.stat}
        </div>
      </div>
      ...
      <div className={`absolute -bottom-10 -right-10 w-32 h-32 bg-gradient-to-br ${service.color} opacity-0 group-hover:opacity-20 blur-3xl transition-opacity duration-500 will-change-contents`} />
    </div>
  </article>
));
...
export default function ProductMatrix() {
  return (
    <section className="relative py-20 px-4 sm:px-6 bg-[#030712] overflow-hidden" aria-labelledby="matrix-title">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full bg-[radial-gradient(circle_at_center,#1e293b_0%,transparent_70%)] opacity-20 pointer-events-none" aria-hidden="true" />
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        <header className="text-center mb-16">
          ...
          <h2 id="matrix-title" className="text-4xl md:text-6xl font-extrabold text-white mb-6 tracking-tight">
        ...
        <footer className="mt-16 p-8 rounded-3xl bg-gradient-to-r from-blue-600/10 via-transparent to-purple-600/10 border border-white/5 text-center">
          ...
          <button 
            ...
            className="bg-white text-black px-8 py-3 rounded-full font-bold hover:bg-blue-600 hover:text-white transition-all transform hover:scale-105 active:scale-95 focus:ring-2 focus:ring-blue-400 focus:outline-none"
          >
            Get Started Free
          </button>
        </footer>
